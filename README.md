...
- Machine-specific secrets are not stored here. Local repo secrets load through direnv and gitignored env files; automation uses cloud/CI secret stores; 1Password is optional SSH-only support plus explicit legacy rendering.
- When encrypted chezmoi source files are present, bootstrap hydrates the local age key from a 1Password reference once per user/machine.
- Privileged Linux package setup lives in `~/.config/dev/install-apt.sh` after apply. Run it with `--with-1password` and/or `--with-tailscale` only when that machine needs those components.
...
- Published branch overlays: `macos` is shared baseline, `linux` carries Linux runtime overlays (`.bashrc/.profile`, Linux shell/systemd/package bootstrap, ssh remote-access helpers, and Linux-specific tmux behavior).
